...
<?php
    ...
    $id="3, ".$_POST["comb"]; #change this everywhere
...
if($p>0)
    $nod_err="Already applied on one of the dates";
        ...
        if(($diff->format('%d'))<7)
         $nod_err="7 days prior notice needed";
        if($diff>60)
        {
        $nod_err="Only 60 days allowed";
        }
        if(empty($nod_err))
		{
            $day=($diff->format('%d'));
            ...
            $result = mysqli_query($con, $sql);
            $sql = "UPDATE employee SET earned_leaves=earned_leaves-$day WHERE emp_id='$emp_id'";
            $result = mysqli_query($con, $sql);
            if($result)
            {
                ?>
...
<html>
  ...
  <body>
    <div class="container">
      <h1 class="form-title">Apply for Earned leave</h1>
      <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>"  action="earned_leave.php" method="post" enctype="multipart/form-data">
